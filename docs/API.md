...
| GET | `/api/v1/audit` | session or token + org | Query the audit log, paginated, optional `?resource_type=`/`?action=`/`?from=`/`?to=` (RFC3339) filters (`audit.read`) — `400` if `from` is after `to` |
